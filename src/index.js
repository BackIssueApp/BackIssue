...
import { runCvMatch as runCvMatchLib, cacheAndLink, addSeriesFromCv, refreshCvVolume, refreshAllIssueDetails, rankCandidates , rematchMismatched } from './cvmatch.js';
...
const TOOLS = {
  ...
  'rematch-mismatched': { label: 'Re-check mismatched volumes', desc: "Find series whose files go beyond their ComicVine volume's last issue — the sign a same-name mini or one-shot was matched instead of the real run — and re-match them. A clear winner is applied and its files re-linked; anything unclear is left for Fix match on the series page. Manually pinned matches are skipped.", needsCv: true, run: (op) => rematchMismatched(db, cvClient(), op) },
