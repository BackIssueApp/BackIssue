<script>
  ...
  // Show each source's config only when enabled, swap client credential fields
  // to the selected client, hint its default port, and let plugins update their
  // own source UI + report whether their source is enabled (for the warning).
  function syncSourceUI() {
    if (!root) return;
    const usenet = root.querySelector('#set-usenetEnabled').checked;
    const cfg = root.querySelector('#usenet-config');
    cfg.classList.toggle('open', usenet);
    ...
    tcfg.classList.toggle('open', torrent);
    ...
    root.querySelector('#no-sources-warning').hidden = usenet || torrent || pluginEnabled.some(Boolean);
  }
...
<main id="settings-page" class="scan-page settings-page" bind:this={root} oninput={markDirty} onchange={markDirty}>
  ...
  <div class="settings-body">
    ...
    <section class="settings-section">
      <p class="modal__subhead">Library</p>
      ...
      <p class="modal__subhead modal__subhead--sub">Notifications</p>
      ...
      <p class="modal__note">POSTed for the categories below (Discord-compatible JSON: <code>{'{ content, type, category, level, … }'}</code>). Also a post-event hook — point it at anything that should react (e.g. a reader-scan trigger). Blank = off. The in-app bell always records everything regardless.</p>
    ...
    <section class="settings-section">
      <p class="modal__subhead">Downloading</p>
    ...
    <section class="settings-section">
      <p class="modal__subhead">Metadata (ComicVine)</p>
      <label class="field">
      ...
      <!-- TEMPORARY: enrichment disabled — the "Use enriched metadata" checkbox
           is hidden while we sort out reliable access to the enrichment source.
           The value is also force-off server-side (settings.js applySettings).
           To restore, un-comment this and remove that override. -->
    ...
    <section class="settings-section">
      ...
      <p id="no-sources-warning" class="modal__note src-warning" hidden><Icon name="alert-triangle" /> No download sources are enabled — new comics can't be downloaded until you turn one on.</p>

      <div id="source-priority" class="source-priority" hidden={sourceOrder.length < 2}>
